...
# zzh
class MyData_Caption_DataLoader(Dataset):
    """my dataset loader."""
    def __init__(
            self,
            csv_path,
        ...
        video_ids = [self.data['sentences'][idx]['video_id'] for idx in range(len(self.data['sentences']))] # zzh: sentence contains video_id as well
        # split_dict = {"train": video_ids[:6513], "val": video_ids[6513:6513 + 497], "test": video_ids[6513 + 497:]}
        split_dict = {"train": video_ids, "val": video_ids, "test": video_ids} # zzh:for eval, all data input
        choiced_video_ids = split_dict[split_type]
        print(choiced_video_ids)
        # TODO
        self.sample_len = 0
        self.sentences_dict = {}
